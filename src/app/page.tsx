...
interface ChatMessage {
  ...
  role: "user" | "model";
  content: string;
  // client-side metadata for the UI
  streaming?: boolean;
  error?: boolean;
  latencyMs?: number;
  receivedAt?: number;
...
function truncateContext(messages: ChatMessage[], maxChars: number): ChatMessage[] {
  // Keep only the most recent messages that fit within maxChars.
...
export default function Home() {
  ...
  const sendMessage = useCallback(
    async (text?: string) => {
      ...
      // Build the conversation to send (exclude the empty placeholder).
      ...
      try {
        ...
        while (true) {
          ...
          // Detect the error sentinel we encode server-side.
          ...
          // Update the assistant message with whatever we have so far.
          setMessages((prev) =>
            prev.map((m) =>
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantMsg.id
              ? {
                  ...m,
                  // If we got no text before the error, surface the error
                  // inline so the user sees what went wrong, not an empty bubble.
                  content:
                    trimmedAcc.length > 0
                      ? acc
        ...
        if (err instanceof DOMException && err.name === "AbortError") {
          // User cancelled — leave the partial text in place.
        } else {
          setError(message);
          setMessages((prev) =>
            prev.map((m) =>
...
// Add a minimal footer link component for the deploy source.
